...
## data significantly different between VD and CSD
...
png("R_plots/ReactionAbundance_ByGroup.png", width = 16, height = 8, units = 'in', res = 300)
...
#### reaction presence ####
data=read.csv("Exported_results/ByGroup_ReactionPresence.csv",header=T,row.names=1,check.names = FALSE,stringsAsFactors=T)

#### read reaction subsystems
annotations=read.csv("Exported_results/ByGroup_ReactionPresenceSubsystems.csv",header=T,row.names=1, check.names = FALSE, stringsAsFactors=T)[c("Subsystem")]
...
t <- data.matrix(dataReduced)
h4 <-Heatmap(t,col=mycol,show_column_names = FALSE,column_title = "One year",
             name = "One year",border = "grey", top_annotation=column_ha,
            show_row_names = FALSE, # right_annotation=row_ha
)

ht_list <- h1+h2+h3+h4

png("R_plots/ReactionPresence_ByGroup.png", width = 16, height = 8, units = 'in', res = 300)
